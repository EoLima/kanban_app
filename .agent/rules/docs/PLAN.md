...
### Tasks Checklist
- [ ] Initialize NestJS backend in a new directory named `backend/`.
- [ ] Install necessary dependencies for NestJS (express, static-serve config).
- [ ] Set up static serving in NestJS using `@nestjs/serve-static` to serve static assets from the frontend static export folder.
- [ ] Create a placeholder endpoint `GET /api/hello` returning a dummy JSON object `{"message": "Hello from Backend"}`.
- [ ] Write a Dockerfile at the project root supporting building the Next.js frontend, copying assets into NestJS static path, and building/exposing the NestJS server.
- [ ] Create shell and batch start/stop scripts in a `scripts/` directory:
...
### Tasks Checklist
- [ ] Update `frontend/next.config.ts` to output a static export (`output: 'export'`).
- [ ] Adjust Dockerfile to build the Next.js static assets and copy them into NestJS's serving folder.
- [ ] Update NestJS configuration to serve the dynamic Next.js bundle at `/`.
- [ ] Create frontend build validation scripts to run in the container pipeline.
...
### Tasks Checklist
- [ ] Design and build a beautiful, premium login interface matching the core navy/purple/yellow color scheme.
- [ ] Add route guarding or local conditional view injection in Next.js:
  - If a user session is absent, display the Login panel.
  - If a user session is present, show the Kanban Board.
- [ ] Create backend endpoint `POST /api/auth/login` validating credentials against `"user"` and `"password"`. On success, set a session cookie or token.
- [ ] Create backend endpoint `POST /api/auth/logout` to clear session tokens/cookies.
- [ ] Add a Sign Out option on the Kanban Board dashboard header.
...
### Tasks Checklist
- [ ] Initialize Prisma ORM in the `backend/` project configuring it to use a local SQLite database (`dev.db`).
- [ ] Define the Prisma model schema representing the following entities:
...
- [ ] Save schema configuration and export schema summary to `docs/DB_SCHEMA.md`.
...
### Tasks Checklist
- [ ] Set up automated database verification in the backend application startup lifecycle:
...
- [ ] Implement NestJS controllers and services with CRUD endpoints for:
...
- [ ] Apply session verification guards to all `/api/board` and `/api/cards` endpoints.
- [ ] Write integration test suites for NestJS controllers using mock request context.
...
### Tasks Checklist
- [ ] Replace mock frontend board state loading with an API request to `GET /api/board` upon initial load.
- [ ] Update frontend column renaming inputs, card creation, and card deletion to fire API requests.
- [ ] Integrate `@dnd-kit` event handlers to fire `PUT /api/board/move` calls when dragging card items.
- [ ] Add smooth optimistic state rendering during card movements and details updating.
- [ ] Implement subtle loading overlays and error boundaries.
...
### Tasks Checklist
- [ ] Implement an AI integration service in the NestJS backend.
- [ ] Parse `OPENROUTER_API_KEY` from environment.
- [ ] Implement connectivity validation route `POST /api/ai/test`. This sends a simple mathematical request (e.g. `2+2=`) to OpenRouter targeting the `google/gemini-2.5-flash:free` model.
- [ ] Set proper referer headers (`HTTP-Referer` and `X-Title`) to register the request correctly.
...
### Tasks Checklist
- [ ] Define system prompts for `google/gemini-2.5-flash:free` passing:
  - Current board structure JSON (all columns, cards, and keys).
  - User's text prompt request.
  - Structured output JSON schema.
- [ ] Enforce the model to return a structured JSON response matching:
  ```json
  {
    "type": "object",
    "properties": {
      ...
      "updates": {
        ...
        "items": {
          "type": "object",
          "properties": {
            "action": { "type": "string", "enum": ["CREATE", "EDIT", "MOVE", "DELETE"] },
            "cardId": { "type": "string" },
            "title": { "type": "string" },
            "details": { "type": "string" },
  ...
  ```
- [ ] Implement route `POST /api/ai/chat` passing user message context, message history, and board payload.
- [ ] Parse the returned structured JSON. Apply the updates (card creations, movements, deletions, details edits) directly to the database.
- [ ] Create mock integration tests for the AI parser.
